...
pct_plot_clean <- function(filtered_data,
                          age_col_name = NULL,
  ...
  if (!is.null(ed_aoo)) {
    ...
    static_data$ed_onset_date <- dob + lubridate::days(as.integer(ed_aoo_days))
  ...
  if (!is.null(age_col_name)) {
    dynamic_data <- dynamic_data %>%
      ...
      mutate(date_assessed = dob + lubridate::days(as.integer(age_days)))
  }
  ...
  dynamic_data <- dynamic_data %>%
    select(date_assessed, age_days, weight_lbs, weight_kgs, height_in, height_cm)
