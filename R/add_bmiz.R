#' Detect how sex variable is coded and convert it to 1/2.
#'
#' This function detects the coding of the sex variable and converts it to numeric values (1 for Male, 2 for Female).
#'
#' @param sex A vector containing the sex variable with different codings.
#'
#' @return A list with 'coding' indicating the coding type and 'sex' with numeric values.
#'
#' @examples
#' align_sex_coding(c("Male", "Female"))  # Returns list(coding = "Male/Female", sex = c(1, 2))
#' @import dplyr
#' @export
align_sex_coding <- function(sex) {
  if (all(sex %in% c("Male", "Female"))) {
    ...
    stop("Invalid coding. Please use 'Male/Female', 'M/F', '0/1 [0=M, 1 = F]', or '1/2 [1 = M, 2 = F]'")
  }
